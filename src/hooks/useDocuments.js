...
export const useDocuments = () => {
  ...
  const duplicateDocument = (docId) => {
    ...
    docToDuplicate.sections.forEach(section => {
      idMapping[section.id] = generateId();
    });

    const duplicatedSections = docToDuplicate.sections.map(section => ({
      ...section,
      id: idMapping[section.id],
      parentId: section.parentId ? idMapping[section.parentId] : null
    }));
